...
def destroy_disk_metadata(dev, node_uuid, quiet_mode=False):
    ...
    :param dev: Path for the device to work on.
    :param node_uuid: Node's uuid. Used for logging.
    :param quiet_mode: set to True and cleaning failures will be transient
    """
    ...
    try:
        ...
        LOG.debug("DBG_NORDIX: DISK TYPE: %{type}s ", {'type': dev_type})
    ...
    if dev_type != 'disk':
        LOG.debug("DBG_NORDIX: NOT DISK WON'T CLEAN: %{nodisk}s",
                  {'nodisk': dev})
        return
    ...
    try:
        ...
        raise errors.DeploymentError(
              ...
              "Error: %(error)s", {'device': dev, 'error': e})
